...
public partial class bao_theloai : System.Web.UI.Page
{
    ...
    {
        string text = "SELECT * FROM BaiBao WHERE MaTheLoai = '" + idBao + "'";
        try
        {
            ...
            txtTenTheLoai.Text = "<i>" + idBao + "</i>";
            foreach (DataRow row in dataTable.Rows)
            {
           
                string Image = row["Image"].ToString();
                string Title = row["Title"].ToString();
                string MoTaNgan = row["MoTaNgan"].ToString();
                string NgayDang = row["NgayDang"].ToString();
                string item = "<div class=\"w3-row w3-margin\"><div class=\"w3-third\"><img src=\"/public/image/"+
                       Image
                       +"\" style=\"width: 100%;\"> </div><div class=\"w3-twothird w3-container\"><h2>"+Title
                           +"</h2><p>" +
              MoTaNgan  +"</p> </div></div>";
